...
# Script to install Shadowbox on a Yandex Cloud instance
...
function cloud::public_ip() {
  curl -H "Metadata-Flavor: Yandex" "http://169.254.169.254/latest/meta-data/public-ipv4"
}
...
function log_for_sentry() {
  echo "[$(date "+%Y-%m-%d@%H:%M:%S")]" "yandex_install_server.sh" "$@" >> "${SENTRY_LOG_FILE}"
}
...
# Applies a guest attribute to the Yandex Cloud instance.
function cloud::set_guest_attribute() {
  local label_key="$1"
  local label_value="$2"

  local GUEST_ATTIBUTE_NAMESPACE="outline"
  local SET_GUEST_ATTRIBUTE_URL="http://169.254.169.254/latest/meta-data/instance/guest-attributes/${GUEST_ATTIBUTE_NAMESPACE}/${label_key}"
  curl -H "Metadata-Flavor: Yandex" -X PUT -d "${label_value}" "${SET_GUEST_ATTRIBUTE_URL}"
}

cloud::set_guest_attribute "install-started" "true"

# Enable BBR.
# Recent Yandex Cloud images are based on Ubuntu 18 and have kernel 4.15+.
